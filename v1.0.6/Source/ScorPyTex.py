...
def clt(nt):
	global title
	if type(nt) == str: nt = path.basename(nt)
	try:
		if not nt == '':
			root.title('ScorPyTex v1.0.6' + ' - ' + nt)
		else:
			root.title('ScorPyTex v1.0.6')
		title = nt
	except:
...
def sssv(event = None):
	...
	if not title == '':
		...
		ssv()
def pdf(title):
	if path.splitext(title)[1] == '.tex':
		pdf_ = path.splitext(title)[0]
	else:
		pdf_ = title
	pdf_ += '.pdf'
	time.sleep(0.5)
	if not path.exists(path.abspath(pdf_)):
		mb.showerror('Error', 'The pdf could not be shown, there could have been an error in your code? I will show the log file.')
		sl()
		return
	else:
...
def sr_(event = None):
	if not title:
		mb.showerror('Error', 'Unnamed document')
		return
	sv(title)
	runtex('pdf')
def sr(event = None):
...
def ld(nm):
	...
	try:
		if type(nm) == str: os.chdir(path.dirname(nm))
	except:
		pass
	clt(nm)
def llld(event = None):
...
def he():
	global type_
	type_.tag_remove('equation', '1.0', 'end')
	start = '1.0'
	while True:
		...
		if not end:
			break
		type_.tag_add('equation', start, end + '+14c')
		start = end
	type_.tag_config('equation', background = 'lightblue')
def ht():
	global type_
	type_.tag_remove('tabular', '1.0', 'end')
	start = '1.0'
	while True:
		...
		if not end:
			break
		type_.tag_add('tabular', start, end + '+13c')
		start = end
	type_.tag_config('tabular', background = 'yellow')
def hs():
	global type_
...
def hoe():
	global type_
	type_.tag_remove('oddsends', '1.0', 'end')
	n = '1.0'
	search = r'\\\\'
	while True:
		count = tk.IntVar()
	...
	while True:
		...
		type_.tag_add('oddsends', n, nn)
		n = nn
	type_.tag_config('oddsends', foreground = 'white', background = 'grey')
def hsb():
	global type_
...
type_ = tk.Text(root, yscrollcommand = scrlbr.set, width = 500, height = 500, font = ('TkDefaultFont', 15))
...
fm.add_separator()
fm.add_command(label = 'Run LuaLaTeX', command = sr)
fm.add_command(label = 'Run PdfLaTeX', command = sr_)
...
m.add_separator()
m.add_command(label  = 'Run LuaLaTeX', command = sr)
m.add_command(label = 'Run PdfLaTeX', command = sr_)
m.add_command(label = 'show log', command = sl)
if len(sys.argv) > 1:
	ld(path.abspath(sys.argv[1]))
type_.focus()
